...
file_put_contents($logFile, date('Y-m-d H:i:s') . " - Raw payload: " . $payload . "\n", FILE_APPEND);
...
if ($data === null && json_last_error() !== JSON_ERROR_NONE) {
    file_put_contents($logFile, date('Y-m-d H:i:s') . " - JSON decoding error: " . json_last_error_msg() . "\n", FILE_APPEND);
...
if (empty($data)) {
    file_put_contents($logFile, date('Y-m-d H:i:s') . " - Error: Payload is empty or JSON decoding failed.\n", FILE_APPEND);
